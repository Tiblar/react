...
function VideoLikes(props) {
    ...
    function fetch() {
        ...
        store.dispatch(fetchPosts(`/users/profile/${props.match.params.username}/likes/video`, params, "likes"));
    }

    let posts = [];
    props.social.posts.map((post) => {
        posts.push(<VideoCard key={post.id} video={post} horizontal={true} />);
    });
...
export default connect(mapStateToProps)(VideoLikes);
